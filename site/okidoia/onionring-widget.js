...
for (i = 0; i < sitesOkidoia.length; i++) {
  if (thisSite.startsWith(sitesOkidoia[i])) { //we use startswith so this will match any subdirectory, users can put the widget on multiple pages
    thisIndex = i;
...
<table>
  ...
  previousIndex = (thisIndex-1 < 0) ? sitesOkidoia.length-1 : thisIndex-1;
  nextIndex = (thisIndex+1 >= sitesOkidoia.length) ? 0 : thisIndex+1;
  ...
  <table>
    <tr>
      ...
      <td class='webring-next'><a href='${sitesOkidoia[nextIndex]}'>next →</a></td>
    </tr>
  </table>
  `);
